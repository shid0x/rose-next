...
counts. The Champion is the bigger single hit; **the Knight is the one with area
damage**, and has two exclusive AoEs where the Champion has none.
...
Where the survivability actually comes from
-------------------------------------------
Honestly: not from this pass. The Soldier tree's defensive kit is **shared**
(class 41) and already strong -- Harden Body is +2..59% max HP over twenty ranks,
Armor Mastery +2..27% DEF, Endure +16..50% DEF, Shield Barrier +9..40% RES, and
Berserk +10..34% ATK with a little DEF. A Knight is durable because it stacks
those, and so is a Champion.

What is *exclusive* to the Knight is control and crowd damage: the only stun in
the line, and the only two area attacks. That is the honest distinction, and it
is enough for a build -- but this pass does not, and should not, claim to have
made the Knight tanky. It made it able to kill what it survives.
...
WRITTEN = (SK_POWER, SK_SUCCESS, SK_DURATION, SK_RELOAD)
RANKS = 10

# base row -> (label, SKILL_1LEV_INDEX, first rank)
FAMILIES = {
    511: ("Impact Wave", 511, 1),
    521: ("Lightning Crasher", 521, 1),
    531: ("Holy Blood", 531, 1),
}
