"""Put level-200+ monster DEF back on the curve the rest of the game follows.
...
Rules:
  * DEF is only ever LOWERED. Anything already at or below the trend is left
...
Usage:
    python scripts/rebalance-endgame-defence.py --dry-run
    python scripts/rebalance-endgame-defence.py
    python scripts/rebalance-endgame-defence.py --verify
    python scripts/rebalance-endgame-defence.py --restore
"""
import argparse
...
SIDECAR = os.path.join(ROOT, "data", "3DDATA", "STB", "LIST_NPC.endgame-def.json")

COL_LEVEL, COL_HP, COL_DEF = 7, 8, 11
...
def fit_trend(stb):
    """Least-squares DEF-vs-level over ten-level bands BELOW FROM_LEVEL.
...
def main():
    ...
    if args.restore:
        ...
        for row, old in saved.items():
            stb.set(row, COL_DEF, str(old))
        with open(NPC_STB, "wb") as fh:
            fh.write(stb.to_bytes())
        os.remove(SIDECAR)
        print(f"restored DEF on {len(saved)} rows; sidecar removed")
        return

    slope, intercept, pts = fit_trend(stb)
    ...
    if args.verify:
        ...
        for row in saved:
            ...
            if gi(stb, row, COL_DEF) != min(saved[row], round(trend(lv) * mult)):
                bad.append(row)
    ...
    for i, lv, cur, cap, boss in changed:
        assert gi(chk, i, COL_DEF) == cap, (i, gi(chk, i, COL_DEF), cap)
