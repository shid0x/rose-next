...
The `/dps` chat command (intercepted locally, never sent) toggles `CDamageMeterPanel` — a draggable DPS/damage overlay with Damage Done (self + party ranking), My Skills, and Damage Taken views. Data comes from `CDamageMeter` (`src/client/gamedata/`), fed by a read-only tap at the single chokepoint `CObjCHAR::PushCombatDamageEvent`; it observes the authoritative combat event stream and never mutates presentation state. Party members' damage is metered for free because combat events broadcast to the 9 surrounding sectors. Skill attribution: exact from legacy `GSV_DAMAGE_OF_SKILL`, best-effort (caster active skill) for FlatBuffer projectile events, `CombatSwing` = auto-attack. **No server or wire changes.** See client `CLAUDE.md` "Damage Meter".
