...
- **`common` and `lib_util` used to compile with `/Od` in the *release* configuration** — an explicit `<Optimization>Disabled</Optimization>` in their release `ItemDefinitionGroup`, not an inherited default. Both are now `MaxSpeed`. That library holds the STB parser, `BinaryReader`, `calculation.cpp` and the shared item/quest code, and it links into the client *and* all three servers, so everything built on it ran unoptimized. Measured on `item_drop.stb` (438 KB, 188,751 cells): parse **34 ms → 2 ms, 17x**, which took the character-select stall's table loading with it.
