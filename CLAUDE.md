...
The `/dps` chat command (intercepted locally, never sent) toggles `CDamageMeterPanel` — a draggable DPS/damage overlay with Damage Done (self + party ranking), My Skills, and Damage Taken views. Data comes from `CDamageMeter` (`src/client/gamedata/`), fed by a read-only tap at the single chokepoint `CObjCHAR::PushCombatDamageEvent`; it observes the authoritative combat event stream and never mutates presentation state. Party members' damage is metered for free because combat events broadcast to the 9 surrounding sectors. Skill attribution: exact from legacy `GSV_DAMAGE_OF_SKILL`, exact from the `DamageEvent.skill_id` wire field (phase 2: display metadata only — presentation logic must not branch on it) for FlatBuffer skill events, `CombatSwing` = auto-attack; the client keeps a caster-active-skill heuristic as fallback when the wire field is 0. Cart/castle-gear attacks fold to the rider via `IsPET()` (castle gear is `OBJ_CGEAR`, not `OBJ_CART` — an equality check silently drops gear damage). See client `CLAUDE.md` "Damage Meter".
