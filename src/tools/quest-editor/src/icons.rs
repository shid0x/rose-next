...
impl IconStore {
    ...
    /// Total number of icons across all sheets (the valid icon_no range).
    pub fn icon_count(&self) -> usize {
        self.tsi
            .as_ref()
            .map(|t| t.sprite_sheets.iter().map(|s| s.sprites.len()).sum())
            .unwrap_or(0)
    }
    ...
    fn try_build_texture(
        ...
        let sheet = match self.ensure_sheet(sprite.sheet_idx) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("ensure_sheet({}) failed: {}", sprite.sheet_idx, e);
                return None;
            }
        };
