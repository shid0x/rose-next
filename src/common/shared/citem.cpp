...
tagBaseITEM::Init(int iItem, unsigned int uiQuantity) {
    ...
    if (1001 > iItem)
        return;
    this->Init(static_cast<unsigned int>(iItem / 1000),
        static_cast<unsigned int>(iItem % 1000),
        uiQuantity);
}

void
tagBaseITEM::Init(unsigned int item_type, unsigned int item_no, unsigned int uiQuantity) {
    this->Clear();

    if (!IsValidITEM(item_type, item_no)) {
        return;
    }

    this->m_cType = static_cast<unsigned short>(item_type);
    this->m_nItemNo = static_cast<unsigned short>(item_no);
    if (this->IsEnableDupCNT()) {
