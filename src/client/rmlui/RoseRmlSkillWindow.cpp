...
RoseRmlSkillWindow::UpdateTooltip() {
    ...
    /// Beside the window, never over it ( legacy tooltips draw before the
    /// RmlUi pass ): to the right, or to the left when there is no room.
    POINT ptMouse;
    CGame::GetInstance().Get_MousePos(ptMouse);
    ...
    /// The side follows where the WINDOW is, not whether this particular
    /// tooltip fits: deciding per tooltip flipped wide ones to the other side.
    POINT pt;
