...
public:
    ...
    void SetRuntimeBudget(float fEmitScale, int iMaxParticles, int iUpdateIntervalMs);
    void SetBatchRenderHint(bool bEnabled);
    int GetConfiguredCapacity();
    int GetRuntimeCapacity();
    int GetActiveCount();
    float GetConfiguredEmitRate();
};

enum EParticleEffectTier {
    PARTICLE_EFFECT_TIER_FULL = 0,
    PARTICLE_EFFECT_TIER_REDUCED,
    PARTICLE_EFFECT_TIER_MINIMAL,
    PARTICLE_EFFECT_TIER_OFF,
    PARTICLE_EFFECT_TIER_COUNT
};
...
private:
    ...
    bool m_bIsVisible;
    EParticleEffectTier m_ParticleTier;
...
public:
    ...
    void SetParticleTier(EParticleEffectTier tier);
    EParticleEffectTier GetParticleTier() { return m_ParticleTier; }
    int GetParticleEmitterCount();
    int GetConfiguredParticleCapacity();
    int GetRuntimeParticleCapacity();
    int GetActiveParticleCount();
    float GetConfiguredParticleEmitRate();
    void SetParticleBatchRenderHint(bool bEnabled);
