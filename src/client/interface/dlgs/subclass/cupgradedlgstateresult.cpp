...
CUpgradeDlgStateResult::Update(POINT ptMouse) {
    ...
    DWORD dwElapsed = dwCurrTime - m_dwStartTime;

    int iValue;
    bool bDone;
    if (dwElapsed >= kRefineDurationMs) {
        iValue = m_iTargetValue;
        bDone = true;
    } else {
        iValue = (int)((INT64)m_iTargetValue * dwElapsed / kRefineDurationMs);
        bDone = false;
    }
    m_pResultGuage->SetValue(iValue);

    if (iValue >= m_iVirtualSuccessPoint)
