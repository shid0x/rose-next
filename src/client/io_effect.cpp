...
void
CParticle::SetRuntimeBudget(float fEmitScale, int iMaxParticles, int iUpdateIntervalMs) {
    if (m_hParticle)
        ::setParticleRuntimeBudget(m_hParticle, fEmitScale, iMaxParticles, iUpdateIntervalMs);
}

void
CParticle::SetBatchRenderHint(bool bEnabled) {
    if (m_hParticle)
        ::setParticleBatchHint(m_hParticle, bEnabled ? 1 : 0);
}

int
CParticle::GetConfiguredCapacity() {
    return m_hParticle ? ::getParticleConfiguredCapacity(m_hParticle) : 0;
}

int
CParticle::GetRuntimeCapacity() {
    return m_hParticle ? ::getParticleRuntimeCapacity(m_hParticle) : 0;
}

int
CParticle::GetActiveCount() {
    return m_hParticle ? ::getParticleActiveCount(m_hParticle) : 0;
}

float
CParticle::GetConfiguredEmitRate() {
    return m_hParticle ? ::getParticleConfiguredEmitRate(m_hParticle) : 0.0f;
}
...
CEffect::CEffect() {
    ...
    m_bIsVisible = false;
    m_ParticleTier = PARTICLE_EFFECT_TIER_FULL;
    m_pListNODE = NULL;
...
CEffect::StartEffect() {
    ...
    SetParticleTier(m_ParticleTier);
}
...
void
CEffect::SetParticleTier(EParticleEffectTier tier) {
    m_ParticleTier = tier;

    if (!m_pEffectDATA || !m_pParticle)
        return;

    for (int iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++) {
        const int configuredCapacity = m_pParticle[iC].GetConfiguredCapacity();
        int maxParticles = 0;
        ...
        switch (tier) {
            case PARTICLE_EFFECT_TIER_REDUCED:
                emitScale = 0.35f;
                maxParticles = max(8, (int)(configuredCapacity * 0.40f));
                break;
            case PARTICLE_EFFECT_TIER_MINIMAL:
                emitScale = 0.08f;
                maxParticles = min(6, max(2, (int)(configuredCapacity * 0.12f)));
                updateIntervalMs = 150;
                break;
            case PARTICLE_EFFECT_TIER_OFF:
                m_pParticle[iC].SetRuntimeBudget(0.0f, 0, 150);
                ::controlParticle(m_pParticle[iC].GetPARTICLE(), 0);
                continue;
        ...
        m_pParticle[iC].SetRuntimeBudget(emitScale, maxParticles, updateIntervalMs);
        ::controlParticle(m_pParticle[iC].GetPARTICLE(), 1);
    }
}

int
CEffect::GetParticleEmitterCount() {
    return m_pEffectDATA ? m_pEffectDATA->m_iParticleCNT : 0;
}

int
CEffect::GetConfiguredParticleCapacity() {
    int count = 0;
    if (!m_pEffectDATA || !m_pParticle)
        return count;

    for (int iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++)
        count += m_pParticle[iC].GetConfiguredCapacity();
    return count;
}

int
CEffect::GetRuntimeParticleCapacity() {
    int count = 0;
    if (!m_pEffectDATA || !m_pParticle)
        return count;

    for (int iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++)
        count += m_pParticle[iC].GetRuntimeCapacity();
    return count;
}

int
CEffect::GetActiveParticleCount() {
    int count = 0;
    if (!m_pEffectDATA || !m_pParticle)
        return count;

    for (int iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++)
        count += m_pParticle[iC].GetActiveCount();
    return count;
}

float
CEffect::GetConfiguredParticleEmitRate() {
    float rate = 0.0f;
    if (!m_pEffectDATA || !m_pParticle)
        return rate;

    for (int iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++)
        rate += m_pParticle[iC].GetConfiguredEmitRate();
    return rate;
}

void
CEffect::SetParticleBatchRenderHint(bool bEnabled) {
    if (!m_pEffectDATA || !m_pParticle)
        return;

    for (int iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++)
        m_pParticle[iC].SetBatchRenderHint(bEnabled);
}
