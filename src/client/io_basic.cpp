...
CBasicDATA::Load3DDATA() {
    ...
    // Fallback to hardcoded skeletons for pet types the STB did not cover.
    // list_pat_skeleton.stb is not shipped in the data set; without this the
    // cart/CG/mount models would fail to load and "drive cart" would apply
    // the speed change but show no vehicle (see 5a3049c2).
    if (m_SkelPET[PET_TYPE_CART01] == 0) {
        m_SkelPET[PET_TYPE_CART01] = m_SKELETON.Add_SKELETON("3DDATA\\Pat\\Cart\\Cart01.zmd");
    }
    if (m_SkelPET[PET_TYPE_CASTLE_GEAR01] == 0) {
