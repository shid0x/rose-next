...
public:
    // total_attack_power / total_hit_rate intentionally NOT overridden here.
    // SetCallerOBJ writes the owner+skill-scaled values into this->stats, and
    // the inherited CObjCHAR implementation reads from stats (plus status-effect
    // adjustments). A previous override returned uninitialized members and made
    // summons miss or deal floor damage.
    void update_speed() override;
