...
public:
	...
	bool CanBatchRenderAdditive(void) const;
	int AppendRenderVertices(VERTEX_PARTICLE *pVertices, int iMaxParticles);
	int AppendRenderPointVertices(VERTEX_PARTICLE_POINT *pVertices, int iMaxParticles);
	...
	int					GetRuntimeParticleLimit(void) const;
	float				GetAverageEmitRate(void) const;
	void				SetRuntimeBudget(float emit_scale, int max_particles);
	...
	int					GetAlignType(void) const { return (m_iAlignType); }
...
private:
	...
	int									m_iNumParticles;
	int									m_iRuntimeMaxParticles;
	float								m_fRuntimeEmitScale;
