...
struct zz_particle_batch_stats {
	int groups;
	int particles;
	int draw_calls;
	int fallback;
...
public:
	...
	int GetConfiguredParticleCapacity(void) const;
	int GetRuntimeParticleCapacity(void) const;
	float GetConfiguredEmitRate(void) const;
	void SetRuntimeBudget(float emit_scale, int max_particles, int update_interval_ms);
	void SetBatchRenderHint(bool enabled) { m_bBatchRenderHint = enabled; }
	bool GetBatchRenderHint(void) const { return m_bBatchRenderHint; }
	bool CanRenderInParticleBatch(void);

	static void RenderParticleListWithBatching(const std::vector<zz_visible*>& particle_nodes);
	static const zz_particle_batch_stats& GetBatchStats(void) { return s_BatchStats; }
...
private:  
	...
	zz_time runtime_update_interval;
	zz_time runtime_update_accum;
	bool m_bBatchRenderHint;
	...
	static zz_particle_batch_stats s_BatchStats;
