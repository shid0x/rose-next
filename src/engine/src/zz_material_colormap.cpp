...
static void set_alpha_test_if_enabled(zz_material_state& render_state, const zz_material_state& material_state, int alpha_ref)
{
	render_state.alpha_test = material_state.alpha_test;
	if (material_state.alpha_test) {
		render_state.alpha_ref = alpha_ref;
	}
}
...
void zz_material_colormap::_set_pass_diffusemap (void)
{
	...
	if (state.alphavalue < 1.0f) {
		s_state.alpha_blend = true;
		set_alpha_test_if_enabled(s_state, state, 10);
		...
		if (receive_fog) {
			s_state.alpha_blend = true;
			set_alpha_test_if_enabled(s_state, state, 32); // do small alpha test for fogged objects.
			_set_texture_x_diffuse_alpha();
		}
		else {
			_set_texture_alpha();
...
void zz_material_colormap::_set_pass_diffusemap_specularmap (void)
{
	if (state.alphavalue < 1.0f) {
		s_state.alpha_blend = true;
		set_alpha_test_if_enabled(s_state, state, state.alpha_ref);
