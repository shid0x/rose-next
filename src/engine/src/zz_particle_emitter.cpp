/** 
* @file zz_particle_emitter.cpp
* @brief particle_emitter class.
* @author Jiho Choi (zho@korea.com)
* @version 1.0
* @date    30-apr-2003
*
* $Header: /engine/src/zz_particle_emitter.cpp 25    04-12-16 4:19p Choo0219 $
*/

#include "zz_tier0.h"
#include <map>
#include <string>
#include <algorithm>
#include <functional>
#include "zz_algebra.h"
#include "zz_misc.h"
#include "zz_mem.h"
#include "zz_particle_emitter.h"
#include "zz_vfs.h"
#include "zz_vfs.h"
#include "zz_profiler.h"
#include "zz_system.h"
#include "zz_renderer.h"
#include "zz_renderer_d3d.h"
#include "zz_motion.h"
#include "zz_texture.h"

ZZ_IMPLEMENT_DYNCREATE(zz_particle_emitter, zz_animatable)

inline DWORD FtoDW( FLOAT f ) { return *((DWORD*)&f); }

const int MAX_PARTICLE_NUM = 1000;
LPDIRECT3DDEVICE9 zz_particle_emitter::m_pd3dDevice = NULL;
LPDIRECT3DVERTEXBUFFER9 zz_particle_emitter::m_vbParticles = NULL;
LPDIRECT3DVERTEXBUFFER9 zz_particle_emitter::m_vbParticles_point = NULL;
int zz_particle_emitter::vb_ref_count = 0;
zz_particle_batch_stats zz_particle_emitter::s_BatchStats = { 0, 0, 0, 0, 0 };

zz_particle_emitter::zz_particle_emitter() : pos_channel(NULL), zz_animatable()
{
	// default particle system configuration - boring!
	Init();
}

void zz_particle_emitter::Init()
{
	m_vPos = vec3( 0.0f, 0.0f, 0.0f );

	for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); i++) 
	{
		zz_delete (*i);
	}	  
	m_Sequences.clear();

	SetPosRange( zz_minmax<vec3>(vec3(0.0f,0.0f,0.0f),vec3(0.0f,0.0f,0.0f)) );
	  
	SetVBSize( 600 );
	runtime_update_interval = 0;
	runtime_update_accum = 0;
	m_bBatchRenderHint = false;
	stop();

	if (!res.get_ready())
		res.make_ready();
}


zz_particle_emitter::~zz_particle_emitter()
{
	Init();
	invalidate_device_objects();
}


bool zz_particle_emitter::load (zz_vfs * fs, bool loadnow)
{
	Init();

	uint32 iSequenceCnt = 0;
	fs->read_uint32( iSequenceCnt );

	for( uint32 i = 0; i < iSequenceCnt; i++ )
	{
		zz_particle_event_sequence* pParticleEventSequence = zz_new zz_particle_event_sequence();
		pParticleEventSequence->Load( fs );
				
		m_Sequences.push_back( pParticleEventSequence );
	}	

	set_bvolume_type(ZZ_BV_OBB);
	reset_bvolume();
	set_collision_level(ZZ_CL_NONE);
	set_cast_shadow(false);

	if (loadnow) {
		restore_device_objects();
	}

	return true;
}

int zz_particle_emitter::GetConfiguredParticleCapacity(void) const
{
	int count = 0;
	for (std::vector<zz_particle_event_sequence *>::const_iterator i = m_Sequences.begin(); i != m_Sequences.end(); ++i) {
		count += (*i)->GetNumParticles();
	}
	return count;
}

int zz_particle_emitter::GetRuntimeParticleCapacity(void) const
{
	int count = 0;
	for (std::vector<zz_particle_event_sequence *>::const_iterator i = m_Sequences.begin(); i != m_Sequences.end(); ++i) {
		count += (*i)->GetRuntimeParticleLimit();
	}
	return count;
}

float zz_particle_emitter::GetConfiguredEmitRate(void) const
{
	float rate = 0.0f;
	for (std::vector<zz_particle_event_sequence *>::const_iterator i = m_Sequences.begin(); i != m_Sequences.end(); ++i) {
		rate += (*i)->GetAverageEmitRate();
	}
	return rate;
}

void zz_particle_emitter::SetRuntimeBudget(float emit_scale, int max_particles, int update_interval_ms)
{
	runtime_update_interval = ZZ_MSEC_TO_TIME(ZZ_MAX(0, update_interval_ms));
	runtime_update_accum = 0;

	for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); ++i) {
		(*i)->SetRuntimeBudget(emit_scale, max_particles);
	}
}

bool zz_particle_emitter::CanRenderInParticleBatch(void)
{
	if (!m_bBatchRenderHint || !IsRunning() || m_Sequences.empty())
		return false;
	if (znzin->renderer->get_render_where() == ZZ_RW_SHADOWMAP)
		return false;

	for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); ++i) {
		if (!(*i)->CanBatchRenderAdditive())
			return false;
		if ((*i)->GetRuntimeParticleLimit() > MAX_PARTICLE_NUM)
			return false;
	}
	return true;
}

bool zz_particle_emitter::load (const char* fname, bool loadnow)
{
	zz_vfs fs;

	if( fs.open( fname, zz_vfs::ZZ_VFS_READ ) == false )
	{
		return false;
	}

	return load(&fs, loadnow);
}

bool zz_particle_emitter::restore_device_objects ()
{  
	HRESULT hr;

	m_pd3dDevice = ((zz_renderer_d3d *)(znzin->renderer))->get_device();

	zz_render_state * state = znzin->get_rs();

	DWORD usage = D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY;

	if (!state->use_hw_vertex_processing_support) {
		usage |= D3DUSAGE_SOFTWAREPROCESSING;
	}

	// create vertex buffer
	if (m_vbParticles_point == NULL) {
		if(FAILED(hr = m_pd3dDevice->CreateVertexBuffer(MAX_PARTICLE_NUM * sizeof( VERTEX_PARTICLE_POINT ),
			usage | D3DUSAGE_POINTS, 
			D3DFVF_PARTICLE_POINT, D3DPOOL_DEFAULT, &m_vbParticles_point, NULL ))) 
		{
			return false;
		}
	}
	if (m_vbParticles == NULL) {
		if(FAILED(hr = m_pd3dDevice->CreateVertexBuffer(MAX_PARTICLE_NUM * 6 * sizeof( VERTEX_PARTICLE ),
			usage, 
			D3DFVF_PARTICLE, D3DPOOL_DEFAULT, &m_vbParticles, NULL ))) 
		{
			return false;
		}
	}

	// restore device objects for each event sequence
	for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); i++) 
	{
		(*i)->RestoreDeviceObjects( m_pd3dDevice );
	}

	// add ref
	vb_ref_count++;

	//ZZ_LOG("particle_emitter: restore_device_objects(%s, %d)\n", this->get_name(), vb_ref_count);
	assert( m_vbParticles || m_vbParticles_point );

	if (!res.get_created())
		res.make_created();
	return true;
}

bool zz_particle_emitter::invalidate_device_objects ()
{
	if (!res.get_created()) return true;

	// invalidate device objects for each event sequence
	for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); i++) 
	{
		(*i)->InvalidateDeviceObjects();
	}

	vb_ref_count--;
	if (vb_ref_count <= 0) {
		vb_ref_count = 0;
		SAFE_RELEASE(m_vbParticles);
		SAFE_RELEASE(m_vbParticles_point);
		res.make_uncreated();
	}
	//ZZ_LOG("particle_emitter: invalidate_device_objects(%s, %d)\n", this->get_name(), vb_ref_count);
	return true;
}

void zz_particle_emitter::update_animation (bool recursive, zz_time diff_time)
{
	zz_animatable::update_animation(false, diff_time); // non-recursive
}

void zz_particle_emitter::render (bool recursive)
{
//	zz_assert(res.get_created());             //test  particel error 잠시 주석처리..
	assert(m_vbParticles || m_vbParticles_point);

	if (znzin->renderer->get_render_where() == ZZ_RW_SHADOWMAP) return;

	//ZZ_LOG("particle_emitter: render(%s)\n", this->get_name());
	if (IsRunning()) 
	{
		mat4 modelviewTM, modelview_worldTM;
		this->get_modelviewTM(modelviewTM);
		znzin->renderer->set_modelview_matrix(modelviewTM);
		for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); i++) 
		{
			if ((*i)->GetImplementType() == 0) {
				assert(m_vbParticles_point);
				(*i)->Render( m_vbParticles_point, m_iVBSize );
			}
			else {
				assert(m_vbParticles);
				(*i)->Render( m_vbParticles, m_iVBSize );
			}
		}
	}
}

void zz_particle_emitter::RenderParticleListWithBatching(const std::vector<zz_visible*>& particle_nodes)
{
	struct BatchKey {
		zz_texture * texture;
		int implement_type;
		int align_type;
		int src_blend;
		int dest_blend;
		int blend_op;
		int texture_width;
		int texture_height;

		bool operator < (const BatchKey& rhs) const {
			if (texture != rhs.texture)
				return std::less<zz_texture*>()(texture, rhs.texture);
			if (implement_type != rhs.implement_type)
				return implement_type < rhs.implement_type;
			if (align_type != rhs.align_type)
				return align_type < rhs.align_type;
			if (src_blend != rhs.src_blend)
				return src_blend < rhs.src_blend;
			if (dest_blend != rhs.dest_blend)
				return dest_blend < rhs.dest_blend;
			if (blend_op != rhs.blend_op)
				return blend_op < rhs.blend_op;
			if (texture_width != rhs.texture_width)
				return texture_width < rhs.texture_width;
			return texture_height < rhs.texture_height;
		}
	};

	s_BatchStats.groups = 0;
	s_BatchStats.particles = 0;
	s_BatchStats.draw_calls = 0;
	s_BatchStats.fallback = 0;
	s_BatchStats.saved = 0;

	if (particle_nodes.empty())
		return;

	std::map<BatchKey, std::vector<zz_particle_event_sequence *> > batches;
	int old_draw_estimate = 0;

	for (std::vector<zz_visible*>::const_iterator it = particle_nodes.begin(); it != particle_nodes.end(); ++it) {
		zz_visible * vis = *it;
		if (!vis || vis->get_visibility() < 0.001f)
			continue;

		zz_particle_emitter * emitter = NULL;
		if (vis->is_a(ZZ_RUNTIME_TYPE(zz_particle_emitter)))
			emitter = static_cast<zz_particle_emitter*>(vis);

		if (!emitter || !emitter->CanRenderInParticleBatch()) {
			s_BatchStats.fallback++;
			vis->render();
			continue;
		}

		for (std::vector<zz_particle_event_sequence *>::iterator seq_it = emitter->m_Sequences.begin();
			seq_it != emitter->m_Sequences.end();
			++seq_it) {
			zz_particle_event_sequence * sequence = *seq_it;
			const int active_particles = sequence->GetNumActiveParticles();
			if (active_particles <= 0)
				continue;

			BatchKey key;
			key.texture = sequence->GetTexture();
			key.implement_type = sequence->GetImplementType();
			key.align_type = sequence->GetAlignType();
			key.src_blend = sequence->GetSrcBlendMode();
			key.dest_blend = sequence->GetDestBlendMode();
			key.blend_op = sequence->GetBlendOperation();
			key.texture_width = sequence->GetTextureSizeWidth();
			key.texture_height = sequence->GetTextureSizeHeight();

			batches[key].push_back(sequence);

			const int old_chunk_particles =
				(sequence->GetImplementType() == 0) ? emitter->m_iVBSize : ZZ_MAX(1, emitter->m_iVBSize / 6);
			old_draw_estimate += ZZ_MAX(1, (active_particles + old_chunk_particles - 1) / old_chunk_particles);
		}
	}

	if (batches.empty())
		return;

	mat4 modelviewTM;
	particle_nodes[0]->get_modelviewTM(modelviewTM);
	znzin->renderer->set_modelview_matrix(modelviewTM);

	zz_renderer_cached_info& cached = znzin->renderer->get_cached_info();
	cached.invalidate(zz_renderer_cached_info::VERTEX_BUFFER);

	znzin->renderer->set_vertex_shader(ZZ_HANDLE_NULL);
	znzin->renderer->set_pixel_shader(ZZ_HANDLE_NULL);

	znzin->renderer->enable_zbuffer(true);
	znzin->renderer->set_zfunc(ZZ_CMP_LESSEQUAL);
	znzin->renderer->enable_alpha_blend(true, ZZ_BT_NORMAL);
	znzin->renderer->enable_alpha_test(false);
	znzin->renderer->enable_zwrite(false);

	m_pd3dDevice->SetTextureStageState(0, D3DTSS_COLOROP,   D3DTOP_MODULATE);
	m_pd3dDevice->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_DIFFUSE);
	m_pd3dDevice->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TEXTURE);
	m_pd3dDevice->SetTextureStageState(1, D3DTSS_COLOROP,   D3DTOP_DISABLE);
	m_pd3dDevice->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_DIFFUSE);
	m_pd3dDevice->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_TEXTURE);
	m_pd3dDevice->SetTextureStageState(0, D3DTSS_ALPHAOP,   D3DTOP_MODULATE);
	m_pd3dDevice->SetTextureStageState(1, D3DTSS_ALPHAOP,   D3DTOP_DISABLE);

	for (std::map<BatchKey, std::vector<zz_particle_event_sequence *> >::iterator batch_it = batches.begin();
		batch_it != batches.end();
		++batch_it) {
		const BatchKey& key = batch_it->first;
		std::vector<zz_particle_event_sequence *>& sequences = batch_it->second;
		bool drew_group = false;

		m_pd3dDevice->SetRenderState(D3DRS_BLENDOP, key.blend_op);
		m_pd3dDevice->SetRenderState(D3DRS_SRCBLEND, key.src_blend);
		m_pd3dDevice->SetRenderState(D3DRS_DESTBLEND, key.dest_blend);

		key.texture->flush_device(false);
		key.texture->set(0);

		if (key.implement_type == 0) {
			m_pd3dDevice->SetRenderState( D3DRS_POINTSPRITEENABLE, TRUE );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSCALEENABLE,  TRUE );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSIZE_MIN, FtoDW(0.00f) );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSIZE_MAX, FtoDW(128.0f) );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSCALE_A,  FtoDW(0.00f) );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSCALE_B,  FtoDW(0.00f) );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSCALE_C,  FtoDW(1.00f) );
			m_pd3dDevice->SetStreamSource( 0, m_vbParticles_point, 0, sizeof(VERTEX_PARTICLE_POINT) );
			m_pd3dDevice->SetFVF( D3DFVF_PARTICLE_POINT );

			VERTEX_PARTICLE_POINT *pVerticesPoint = NULL;
			if (FAILED(m_vbParticles_point->Lock(0, MAX_PARTICLE_NUM * sizeof(VERTEX_PARTICLE_POINT),
				(void **)&pVerticesPoint, D3DLOCK_DISCARD)))
				continue;

			bool buffer_locked = true;
			int particles_in_chunk = 0;
			for (std::vector<zz_particle_event_sequence *>::iterator seq_it = sequences.begin();
				seq_it != sequences.end();
				++seq_it) {
				zz_particle_event_sequence * sequence = *seq_it;
				const int active_particles = sequence->GetNumActiveParticles();
				if (active_particles <= 0)
					continue;

				if (particles_in_chunk > 0 && particles_in_chunk + active_particles > MAX_PARTICLE_NUM) {
					m_vbParticles_point->Unlock();
					if (SUCCEEDED(m_pd3dDevice->DrawPrimitive(D3DPT_POINTLIST, 0, particles_in_chunk))) {
						s_BatchStats.draw_calls++;
						drew_group = true;
					}
					if (FAILED(m_vbParticles_point->Lock(0, MAX_PARTICLE_NUM * sizeof(VERTEX_PARTICLE_POINT),
						(void **)&pVerticesPoint, D3DLOCK_DISCARD))) {
						buffer_locked = false;
						break;
					}
					particles_in_chunk = 0;
				}

				const int appended = sequence->AppendRenderPointVertices(pVerticesPoint + particles_in_chunk,
					MAX_PARTICLE_NUM - particles_in_chunk);
				particles_in_chunk += appended;
				s_BatchStats.particles += appended;
			}

			if (buffer_locked)
				m_vbParticles_point->Unlock();
			if (buffer_locked && particles_in_chunk > 0) {
				if (SUCCEEDED(m_pd3dDevice->DrawPrimitive(D3DPT_POINTLIST, 0, particles_in_chunk))) {
					s_BatchStats.draw_calls++;
					drew_group = true;
				}
			}
			m_pd3dDevice->SetRenderState( D3DRS_POINTSPRITEENABLE, FALSE );
			m_pd3dDevice->SetRenderState( D3DRS_POINTSCALEENABLE,  FALSE );
		}
		else {
			m_pd3dDevice->SetStreamSource( 0, m_vbParticles, 0, sizeof(VERTEX_PARTICLE) );
			m_pd3dDevice->SetFVF( D3DFVF_PARTICLE );

			VERTEX_PARTICLE *pVertices = NULL;
			if (FAILED(m_vbParticles->Lock(0, MAX_PARTICLE_NUM * 6 * sizeof(VERTEX_PARTICLE),
				(void **)&pVertices, D3DLOCK_DISCARD)))
				continue;

			bool buffer_locked = true;
			int particles_in_chunk = 0;
			for (std::vector<zz_particle_event_sequence *>::iterator seq_it = sequences.begin();
				seq_it != sequences.end();
				++seq_it) {
				zz_particle_event_sequence * sequence = *seq_it;
				const int active_particles = sequence->GetNumActiveParticles();
				if (active_particles <= 0)
					continue;

				if (particles_in_chunk > 0 && particles_in_chunk + active_particles > MAX_PARTICLE_NUM) {
					m_vbParticles->Unlock();
					if (SUCCEEDED(m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, particles_in_chunk * 2))) {
						s_BatchStats.draw_calls++;
						drew_group = true;
					}
					if (FAILED(m_vbParticles->Lock(0, MAX_PARTICLE_NUM * 6 * sizeof(VERTEX_PARTICLE),
						(void **)&pVertices, D3DLOCK_DISCARD))) {
						buffer_locked = false;
						break;
					}
					particles_in_chunk = 0;
				}

				const int appended = sequence->AppendRenderVertices(pVertices + particles_in_chunk * 6,
					MAX_PARTICLE_NUM - particles_in_chunk);
				particles_in_chunk += appended;
				s_BatchStats.particles += appended;
			}

			if (buffer_locked)
				m_vbParticles->Unlock();
			if (buffer_locked && particles_in_chunk > 0) {
				if (SUCCEEDED(m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, particles_in_chunk * 2))) {
					s_BatchStats.draw_calls++;
					drew_group = true;
				}
			}
		}

		if (drew_group)
			s_BatchStats.groups++;
	}

	m_pd3dDevice->SetRenderState(D3DRS_BLENDOP, D3DBLENDOP_ADD);
	s_BatchStats.saved = ZZ_MAX(0, old_draw_estimate - s_BatchStats.draw_calls);
}

void zz_particle_emitter::attach_motion (zz_motion * motion_to_attach)
{
	if (!motion_to_attach) return; // no motion

	zz_animatable::attach_motion(motion_to_attach);

	pos_channel = static_cast<zz_channel_position*>(motion_to_attach->channels[0]);
}

bool zz_particle_emitter::apply_motion (void)
{
	if (!motion) return false; // no motion

	zz_time time = motion_controller.get_current_time();

	int fps = motion->get_fps();
	if (pos_channel) pos_channel->get_by_time(time, fps, &this->position);
	return true;
}

void zz_particle_emitter::build_mesh_minmax (vec3& mesh_min_out, vec3& mesh_max_out)
{
	mesh_min_out = m_vPosRange.m_min;
	mesh_max_out = m_vPosRange.m_max;
}

// modified version for zz_animatable::update_time()
void zz_particle_emitter::update_time (bool recursive, zz_time diff_time)
{
	// default particle does not care about motion_controller.
	// but motion attached paricle emittor can be motion_contrelled.
	if (!motion_controller.update(diff_time)) {
		motion_controller.stop(); // not same as stop()
	}

	if (apply_motion()) { // do apply motion no relation with motion_controller
		invalidate_transform();
	}

	// like update_animation
	m_vPos = this->get_worldTM().get_position();

	vec3 world_scale;

	bool still_alive = false;
	if (IsRunning())
	{
		zz_time particle_diff_time = diff_time;
		if (runtime_update_interval > 0) {
			runtime_update_accum += diff_time;
			if (runtime_update_accum < runtime_update_interval) {
				zz_visible::update_time(recursive, diff_time);
				return;
			}
			particle_diff_time = runtime_update_accum;
			runtime_update_accum = 0;
		}

		for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); i++) 
		{
			(*i)->set_world_tm(&this->get_worldTM());
			this->get_scale_world(world_scale);
			(*i)->set_scale_world(world_scale.x);
			if ((*i)->update(particle_diff_time, m_vPos))
				still_alive = true; // any sequence is still alive
		}
		if (still_alive == false) { // no sequences are still alive
			// not same as stop();
			pause();
			DeleteAllParticles();
		}
	}

	zz_visible::update_time(recursive, diff_time);
}

void zz_particle_emitter::before_render ()
{
	if (!res.get_created()) {
		restore_device_objects();
	}

	if (!IsRunning()) return; // do nothing

	for (std::vector<zz_particle_event_sequence *>::iterator i = m_Sequences.begin(); i != m_Sequences.end(); i++) 
	{
		(*i)->before_render();
	}
}
