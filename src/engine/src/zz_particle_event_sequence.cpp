...
int zz_particle_event_sequence::Reset()
{
	...
	m_iNumParticles = 100;
	m_iRuntimeMaxParticles = -1;
	m_fRuntimeEmitScale = 1.0f;
...
int zz_particle_event_sequence::GetRuntimeParticleLimit(void) const
{
	if (m_iRuntimeMaxParticles < 0)
		return m_iNumParticles;

	return ZZ_MAX(0, ZZ_MIN(m_iRuntimeMaxParticles, m_iNumParticles));
}
...
{
	m_fRuntimeEmitScale = ZZ_MAX(0.0f, emit_scale);
	m_iRuntimeMaxParticles = max_particles;

	const int limit = GetRuntimeParticleLimit();
	if (m_WorldParticles) {
		for (int q = limit; q < m_WorldParticles->GetTotalElements(); ++q) {
			if (m_WorldParticles->IsAlive(q))
				m_WorldParticles->Delete(q);
		}
	}
	if (m_Particles) {
		for (int q = limit; q < m_Particles->GetTotalElements(); ++q) {
			if (m_Particles->IsAlive(q))
				m_Particles->Delete(q);
		}
	}
}
...
bool zz_particle_event_sequence::update (zz_time diff_time, vec3 m_vPartSysPos )
{
	...
	int q;
	const int particle_limit = GetRuntimeParticleLimit();
	if (this->GetUpdateCoord() == ZZ_UPDATE_COORD_WORLD) {
		// update existing particles
		for (q=0 ; q < particle_limit ; q++ ) {
			if ( m_WorldParticles->IsAlive(q) ) 
			{
	...
	// Accumulate this frame's emission budget. The accumulator is consumed
	// only when a spawn actually succeeds (see the while-loop below). Prior
	// behaviour decremented the accumulator unconditionally when it crossed
	// 1.0, even if the spawn was blocked by num_particles cap, which made
	// the spawn-after-death timing drift relative to the death frame and
	// produced a synchronised ~83 ms blank gap on cosmetic emitters with
	// num_particles=1 (e.g. SMOKE_04 chimney smoke in JZ01). This matches
	// the Rust port at
	// rose-offline-client/src/systems/particle_sequence_system.rs:336-394.
	float fEmitRateThisFrame = m_EmitRate.get_random_num_in_range() * m_fRuntimeEmitScale;
	m_fNumNewPartsExcess += fEmitRateThisFrame * fTimeDelta;

	bool is_running = true;
	int remaining_for_loops = -1; // -1 == infinite
	if (m_Loops > 0) {
		const int total_allowed = m_Loops * m_iNumParticles;
		remaining_for_loops = ZZ_MAX(0, total_allowed - m_iTotalParticleLives);
		if (remaining_for_loops == 0 && num_current_active == 0) {
			is_running = false;
		}
		// Don't let the accumulator overshoot the finite emission budget.
		if (m_fNumNewPartsExcess > (float)remaining_for_loops) {
			m_fNumNewPartsExcess = (float)remaining_for_loops;
		}
	}

	int free_elements = ZZ_MAX(0, particle_limit - num_current_active);
	while (m_fNumNewPartsExcess >= 1.0f
		&& free_elements > 0
		&& (remaining_for_loops < 0 || remaining_for_loops > 0))
	{
		try
		{
		...
		m_fNumNewPartsExcess -= 1.0f;
		free_elements--;
		if (remaining_for_loops > 0) {
			remaining_for_loops--;
		}
	}
	SetRuntimeBudget(m_fRuntimeEmitScale, m_iRuntimeMaxParticles);
...
bool zz_particle_event_sequence::CanBatchRenderAdditive(void) const
{
	if (!m_texParticle)
		return false;
	if (m_iImplementType != 0 && m_iImplementType != 1)
		return false;
	return (m_iBlendOperation == D3DBLENDOP_ADD && m_iDestBlendMode == D3DBLEND_ONE);
}

int zz_particle_event_sequence::AppendRenderPointVertices(VERTEX_PARTICLE_POINT *pVertices, int iMaxParticles)
{
	if (!pVertices || iMaxParticles <= 0)
		return 0;

	int iAppended = 0;
	const int particle_limit = GetRuntimeParticleLimit();
	for (int q = 0; q < particle_limit && iAppended < iMaxParticles; q++)
	{
		if ((m_Particles && m_Particles->IsAlive(q)) || (m_WorldParticles && m_WorldParticles->IsAlive(q)))
		{
			...
			if (m_Particles)
				...
				part = &m_WorldParticles->GetAt(q);

			vec3 part_pos;
			const mat4& tm = world_tm ? *world_tm : mat4_id;
			...
			case ZZ_UPDATE_COORD_LOCAL:
				{
					vec4 part_world_pos;
					mult(part_world_pos, tm, vec4(part->m_vPos.x, part->m_vPos.y, part->m_vPos.z, 1.0f));
			...
			case ZZ_UPDATE_COORD_LOCAL_WORLD:
				part_pos = part->m_vPos + tm.get_position();
				break;
			}

			pVertices->position = part_pos;
			pVertices->pointsize = part->m_vSize.x * world_scale;
...
int zz_particle_event_sequence::AppendRenderVertices(VERTEX_PARTICLE *pVertices, int iMaxParticles)
{
	if (!pVertices || iMaxParticles <= 0)
		return 0;
	...
	for (int q = 0; q < particle_limit && iAppended < iMaxParticles; q++)
	{
		if ((m_Particles && m_Particles->IsAlive(q)) || (m_WorldParticles && m_WorldParticles->IsAlive(q)))
		{
			...
			if (part->m_bAngleOnOff) {
				...
				pos[0].set(-x - x2, -y - y2, 0);
				pos[1].set(x - x2, y - y2, 0);
				pos[2].set(-x + x2, -y + y2, 0);
				pos[3] = pos[2];
				pos[4] = pos[1];
				pos[5].set(x + x2, y + y2, 0);
			}
			else {
			...
			uv[0].set((texture_index.x + 0)/texture_size.x, (texture_index.y + 1)/texture_size.y);
			uv[1].set((texture_index.x + 1)/texture_size.x, (texture_index.y + 1)/texture_size.y);
			uv[2].set((texture_index.x + 0)/texture_size.x, (texture_index.y + 0)/texture_size.y);

			uv[3].set((texture_index.x + 0)/texture_size.x, (texture_index.y + 0)/texture_size.y);
			uv[4].set((texture_index.x + 1)/texture_size.x, (texture_index.y + 1)/texture_size.y);
			uv[5].set((texture_index.x + 1)/texture_size.x, (texture_index.y + 0)/texture_size.y);
...
bool zz_particle_event_sequence::Render( LPDIRECT3DVERTEXBUFFER9 pVB, int iVBSize )
{
	...
	const int particle_limit = GetRuntimeParticleLimit();
	for (int q=0; q < particle_limit; q++)
	{
